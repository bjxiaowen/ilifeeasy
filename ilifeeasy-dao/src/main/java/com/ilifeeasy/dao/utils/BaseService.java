package com.ilifeeasy.dao.utils;
...
import com.ilifeeasy.entity.common.BaseEntity;
...
public abstract class BaseService {
	...
	public static String dataScopeFilter(User user, String officeAlias, String userAlias) {
		...
		if (!user.isAdmin()) {
			...
			for (Role r : user.getRoleList()) {
				for (String oa : StringTools.split(officeAlias, ",")) {
					if (!dataScope.contains(r.getDataScope()) && StringTools.isBlank(oa)) {
			...
			if (!isDataScopeAll) {
				if (StringTools.isBlank(userAlias)) {
					for (String ua : StringTools.split(userAlias, ",")) {
		...
		if (StringTools.isBlank(sqlString.toString())) {
	...
	public static void dataScopeFilter(BaseEntity<?> entity, String sqlMapKey, String officeWheres, String userWheres) {
		...
		// 生成部门权限SQL语句
		for (String where : StringTools.split(officeWheres, ",")) {
		...
		// 生成个人权限SQL语句
		for (String where : StringTools.split(userWheres, ",")) {
